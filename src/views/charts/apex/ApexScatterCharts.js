...
const ApexScatterCharts = ({ propsSeries, direction, warning, primary, success }) => {
  ...
  const options = {
    ...
    xaxis: {
      ...
      labels: {
        rotate: false,
        formatter(val, b, c, d) {
          return c && c.i !== undefined ?  ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][c.i] : val
        }
      }
    }
  }
  ...
  return (
    <Card>
      ...
      <CardBody>
        <Chart options={options} series={propsSeries || series} type='scatter' height={400} />
      </CardBody>
    </Card>
  )
}
