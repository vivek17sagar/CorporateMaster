...
const AddEventSidebar = props => {
  ...
  const [value, setValue] = useState([{ value: 'Birthdays', label: 'Birthdays', color: 'primary' }])

  // ** Select Options
  const options = [
    { value: 'Birthdays', label: 'Birthdays', color: 'primary' },
  ...
  const handleResetInputValues = () => {
    ...
    setDesc('')
    setGuests({})
    setValue([{ value: 'Birthdays', label: 'Birthdays', color: 'primary' }])
    ...
    if (!isObjEmpty(selectedEvent)) {
      ...
      const resolveLabel = () => {
        if (calendar.length) {
          ...
          return { value: 'Birthdays', label: 'Birthdays', color: 'primary' }
        }
      }
    ...
    >
      <ModalHeader className='mb-1' toggle={handleAddEventSidebar} close={CloseBtn} tag='div'>
        <h5 className='modal-title'>
          {selectedEvent && selectedEvent.title && selectedEvent.title.length ? 'Update' : 'Add'} Reminder
        </h5>
      </ModalHeader>
